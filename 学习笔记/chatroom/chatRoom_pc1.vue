<template>
  <div class="chatRoomBox">
    <span v-if="!showRoom" class="door" @click="showRoom=true;IsAddRoom=true"><img src="/static/chatRoom/show.png" alt=""></span>
    <div v-if="IsAddRoom" class="chatRoom" :class="{'closeRoom':!showRoom}">
          ...
          <li v-for="(v,i) in data" :key="i" :class="{'right':v.type===1,'systemMessage':v.type===3}">
            <div class="box">
              ...
              <div class="headerImg" v-if="v.type!==3">
                <img :src="v.type===1?'./static/chatRoom/other.png':'./static/chatRoom/mine.png'" alt="">
              </div>
              <!--非系统消息，聊天数据-->
              ...
              <div v-else class="systemItem">
                <div class="systemTitle">系统消息: {{v.title}}</div>
      ...
      <div class="edit">
        <!--这里的v-model不能加修饰符trim，不然当ctrl+enter的时候，添加\n来换行时不起效果，因为被修饰符trim删除了-->
  ...
  function tranlateDate(v) {
	...
	export default {
        ...
        */
        ...
        // 自己加上一条数据
        this.data.push({type:1,time:tranlateDate(new Date()),sendUin:this.subUserName,text:this.message,name:this.currentUser})
