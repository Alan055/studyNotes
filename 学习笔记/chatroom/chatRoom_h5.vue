<template>
  <f7-page class="chatRoomPage" name="chatRoom">
    <f7-navbar sliding>
      <f7-nav-right>
        <f7-link @click="alClick()" class="link"><em class="m_ico rightIcon">&#xe697;</em></f7-link>
      </f7-nav-right>
      <f7-nav-center class="centerTitle" sliding>推送消息</f7-nav-center>
      <f7-nav-right></f7-nav-right>
    </f7-navbar>
    <div class="chatRoom">
      <!--<div class="title">交流室</div>-->
          ...
          <li v-for="(v,i) in data" :key="i" :class="{'right':v.type===1,'systemMessage':v.type===3}">
            <div class="box">
              ...
              <div class="headerImg" v-if="v.type!==3">
                <img :src="v.type===1?imgSrc[0]:imgSrc[1]" alt="">
              </div>
              <!--非系统消息，聊天数据-->
              <div class="info" v-if="v.type!==3">
                <div class="top">
                  ...
                  <!--<span :class="v.type===1?'time':'name'">{{v.type===1?'':v.name}}</span>-->
                  <!--<span :class="v.type===1?'name':'time'">{{v.type===1?v.name:''}}</span>-->
                  <span>{{v.name}}</span>
                </div>
              ...
              <!--系统消息-->
              <div v-else class="systemItem" @click="openNotice(v,i)" :class="{'withdraw':v.messageType===1,'recharge':v.messageType==0,}">
                <div class="systemTitle" :class="{'currentItem':currentItem===i}">系统消息: {{v.title}}<br>{{v.time}}</div>
                <!--这里做成f7通知的效果-->
                <!--点击标题之后会有一个从上面下来的弹框-->
      ...
      <div class="clean" @click="data=[];closeNotice()">清空</div>
      <!--<div class="edit">-->
        <!--&lt;!&ndash;这里的v-model不能加修饰符trim，不然当ctrl+enter的时候，添加\n来换行时不起效果，因为被修饰符trim删除了&ndash;&gt;-->
        <!--<textarea class="text" autofocus  :class="{'close':close}" :disabled="close"-->
...
<script>
	...
	export default {
		...
		data() {
			return {
				imgSrc:['/static/chatRoom/other.png','/static/chatRoom/mine.png'],//
				data: [],//对话列表的数据
        currentItem:-1,//这个是当前选中消息
        currentOption:null,//这是用于存储通知的弹框。
				message: '',//输入框的消息
				ctrlIsDown: false,//ctrl是否被按下，这个用来区别ctrl+enter和enter的键盘事件
				init: false,//对话列表初始化，一开始滚动条是在最上面的，让其到最下面去，发送消息的时候也到最下面去
        ...
        */
				subUserName: window.localStorage.getItem('_last') == 'test0001' ? 'tomtest' : '',//发送出去，接受对象的用户名
				toParent: window.localStorage.getItem('_last') == 'test0001' ? '0' : '1',//websocket往哪儿发送，1是向父级发送  0是往子级发送
				currentUser: window.localStorage.getItem('_last'),
				receiveName: '游客',
			}
		},
		computed: {},
        ...
        this.newM()
        //如果有选中的条数，就加1
        this.currentItem!==-1&&this.currentItem++
			}
		},
		methods: {
			//发送消息事件
      ...
      *  就会在一瞬间里面被认为是抬起了，然后又按下了
      ...
      //打开通知组件
			openNotice(v,i){
				if(this.currentItem!==i){
					this.currentItem=i
          //这个存起来，方便之后关闭使用
					this.currentOption=f7.addNotification({
						title: v.title,
						message: v.text,
      ...
      //手动关闭通知弹框
      closeNotice(){
	      this.currentOption&&f7.closeNotification(this.currentOption)
	      this.currentItem=-1
      },
		},
		created() {
